...
- **Projects and policy** are a numbered index at the reading measure (`partials/entry.njk`), not cards. On hover or focus an entry's rule redraws in ink from the left, the title's underline writes itself, a ↗ slides in and the app icon regains its colour (icons rest in grey). The whole row links to the entry. A policy name of the form `Title (Source)` shows the source as a mono kicker (`splitSource` filter).
